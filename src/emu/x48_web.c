...
/*
 * x48_web.c - Emscripten/Web replacement for x48.c (originally x48_x11.c).
 * Replaces JNI/Android and X11 code with a ring-buffer event system
 * and stub display functions for the web build.
 */
...
/* Global variables from original x48.c */
Display	       *dpy;
int		screen;
unsigned int	depth;
Colormap	cmap;
...
/* Event ring buffer for web input */

#define EVENT_QUEUE_SIZE 64
static unsigned int event_queue[EVENT_QUEUE_SIZE];
...
#endif
void push_key_event(int code) {
    int next = (eq_head + 1) % EVENT_QUEUE_SIZE;
    if (next != eq_tail) {
        event_queue[eq_head] = code;
        eq_head = next;
    }
}
...
#endif
{
    ...
    unsigned int code = event_queue[eq_tail];
    eq_tail = (eq_tail + 1) % EVENT_QUEUE_SIZE;
    if (code < 100) {
        key_event(code - 1, 1);   /* press: code 1-49 → button 0-48 */
    } else {
        key_event(code - 101, 0); /* release: code 101-149 → button 0-48 */
    }
    return 1;
}
