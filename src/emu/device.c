...
/* -----------------------------------------------------------------------
 * Windowed speaker analysis.
 *
 * speaker_instr_count is incremented per-instruction from main_wasm.c.
 * check_out_register() accumulates toggle intervals in a window.
 * JS polls get_speaker_frequency() every ~20ms which returns the
 * frequency and resets the window.  No gap detection needed — during
 * BEEP there are hundreds of toggles per window, during keyboard
 * scanning there are 0-3.
 * ----------------------------------------------------------------------- */

long long speaker_instr_count = 0;  /* incremented externally */

static long long last_toggle_instr = 0;
static long long win_half_sum = 0;
static int win_toggle_count = 0;

/*
 * Native HP-48 effective instruction rate for tight loops.
 * Saturn CPU crystal = 3.6864 MHz, ~22 clocks/instruction in BEEP loop.
 * Empirically: 440 Hz BEEP half-period = 192 instructions.
 *   192 / (1/(2*440)) = 169,014 ips.
 */
#define HP48_IPS 169000LL
...
#endif
{
	...
	if (state != last_state) {
		long long delta = speaker_instr_count - last_toggle_instr;
		last_toggle_instr = speaker_instr_count;

		if (win_toggle_count > 0 && delta > 0) {
			win_half_sum += delta;
		}
		win_toggle_count++;
...
/*
 * Called by JS every ~20ms.  Returns the detected frequency in Hz,
 * or 0 if no tone detected.  Resets the window for the next poll.
 *
 * During BEEP at 440 Hz: ~400 toggles/window → clear frequency.
 * During keyboard scanning: 0-3 toggles/window → returns 0.
 */
EMSCRIPTEN_KEEPALIVE int get_speaker_frequency(void) {
	int count = win_toggle_count;
	long long sum = win_half_sum;

	/* Reset window for next poll */
	win_toggle_count = 0;
	win_half_sum = 0;
	...
	long long avg_half = sum / intervals;
	if (avg_half <= 0) return 0;

	int freq = (int)(HP48_IPS / (2 * avg_half));

	/* Filter: only audible frequencies */
